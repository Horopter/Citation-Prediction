...
#SBATCH --job-name=model_pytorch_mlp
...
if python -c "import sys; import papermill; import ipykernel; import torch; import polars; import sklearn; print('✓ Basic packages available', flush=True)" 2>/dev/null; then
...
KNAME="model_pytorch_mlp-${SLURM_JOB_ID:-$$}"
log "Installing Jupyter kernel: ${KNAME}..."
if python -m ipykernel install --user --name "${KNAME}" --display-name "Model PyTorch MLP (${KNAME})" 2>&1; then
...
NOTEBOOK_IN="src/notebooks/model_pytorch_mlp_all_features.ipynb"
STAMP="$(date +%Y%m%d-%H%M%S)"
NOTEBOOK_OUT="$WORK_DIR/runs/model_pytorch_mlp_executed_${STAMP}.ipynb"
...
pkill -f "papermill.*model_pytorch_mlp" 2>/dev/null || true
pkill -f "ipykernel.*model_pytorch_mlp" 2>/dev/null || true
# Clear previous logs and checkpoints
rm -f "$WORK_DIR/logs/model_pytorch_mlp_all_features_run.log" 2>/dev/null || true
rm -f "$WORK_DIR/data/checkpoints/model_pytorch_mlp*" 2>/dev/null || true
rm -f "$WORK_DIR/models/saved_models/model_pytorch_mlp_all_features_best.pkl" 2>/dev/null || true
rm -f "$WORK_DIR/data/submission_files/submission_model_pytorch_mlp.csv" 2>/dev/null || true
sleep 2
...
LOG_FILE="$WORK_DIR/logs/model_pytorch_mlp_all_features_run.log"
...
if [ -n "${SLURM_TMPDIR:-}" ] && [ "$WORK_DIR" != "$ORIG_DIR" ]; then
    ...
    if [ -d "$WORK_DIR/models/saved_models" ]; then
        ...
        cp -rv "$WORK_DIR/models/saved_models"/model_pytorch_mlp* "$ORIG_DIR/models/saved_models/" 2>/dev/null || true
    fi
...
if [ -f "data/submission_files/submission_model_pytorch_mlp.csv" ] || [ -f "$WORK_DIR/data/submission_files/submission_model_pytorch_mlp.csv" ]; then
    log "✓ Found submission file: submission_model_pytorch_mlp.csv"
...
# Check saved model
if [ -f "models/saved_models/model_pytorch_mlp_all_features_best.pkl" ] || [ -f "$WORK_DIR/models/saved_models/model_pytorch_mlp_all_features_best.pkl" ]; then
    log "✓ Found saved model: model_pytorch_mlp_all_features_best.pkl"
    FILES_FOUND=$((FILES_FOUND + 1))
else
