...
#SBATCH --job-name=data_exploration_next_steps
...
KNAME="data-exploration-next-steps-${SLURM_JOB_ID:-$$}"
log "Installing Jupyter kernel: ${KNAME}..."
if python -m ipykernel install --user --name "${KNAME}" --display-name "Data Exploration Next Steps (${KNAME})" 2>&1; then
...
NOTEBOOK_IN="src/notebooks/data_exploration_next_steps.ipynb"
STAMP="$(date +%Y%m%d-%H%M%S)"
NOTEBOOK_OUT="$WORK_DIR/runs/data_exploration_next_steps_executed_${STAMP}.ipynb"
...
pkill -f "papermill.*data_exploration_next_steps" 2>/dev/null || true
pkill -f "ipykernel.*data-exploration-next-steps" 2>/dev/null || true
# Clear previous logs
rm -f "$WORK_DIR/logs/data_exploration_next_steps_run.log" 2>/dev/null || true
sleep 2
...
LOG_FILE="$WORK_DIR/logs/data_exploration_next_steps_run.log"
